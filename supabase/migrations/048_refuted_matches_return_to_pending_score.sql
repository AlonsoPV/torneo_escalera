-- Refutacion: el partido queda activo en revision admin.
-- El cuadro puede tratarlo como no jugado, pero BD conserva marcador/ganador
-- para que administracion pueda validar tal cual o corregirlo.
...
begin
  ...
  if p_accept then
    raise exception 'La confirmacion del rival ya no esta disponible; el marcador es oficial al registrarse.';
  end if;
  ...
  if m.score_submitted_by is not null then
    if uid = m.score_submitted_by then
      raise exception 'No puedes refutar tu propio envio';
    end if;
  else
  ...
  if p_dispute_reason is null or length(trim(p_dispute_reason)) < 3 then
    raise exception 'Escribe un motivo de rechazo (min. 3 caracteres)';
  end if;
