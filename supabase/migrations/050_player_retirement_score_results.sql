-- Permite reportar partidos 2 de 3 incompletos por retiro desde el flujo de jugador.
-- Casos:
-- - not_reported: -1 / -1, sin marcador ni ganador
-- - retired: gana quien tiene mas games, puntos normales 3 / 1
-- - retired_draw: empate por retiro, 1 / 1, sin ganador
...
begin
  ...
  if v_result_type not in ('normal', 'not_reported', 'retired', 'retired_draw') then
  ...
  if v_result_type in ('retired', 'retired_draw') then
    if v_game_type <> 'best_of_3' then
      raise exception 'El retiro solo aplica para partidos 2 de 3 sets';
    end if;
    ...
    if v_result_type = 'retired' and (
      ...
      raise exception 'Retiro: indica el ganador por games';
    end if;

    if v_result_type = 'retired' and v_a_games = v_b_games then
      raise exception 'Retiro: el marcador esta empatado; usa empate por retiro';
    end if;

    if v_result_type = 'retired'
       and ((v_a_games > v_b_games and p_winner_group_player_id <> m_old.player_a_id)
         or (v_b_games > v_a_games and p_winner_group_player_id <> m_old.player_b_id))
    then
      raise exception 'Retiro: el ganador debe ser quien tiene mas games';
    end if;

    if v_result_type = 'retired_draw' and v_a_games <> v_b_games then
      raise exception 'Retiro: para empate ambos jugadores deben tener la misma cantidad de games';
    end if;
  end if;
  ...
  insert into public.match_score_logs (
    ...
    log_action,
    m_old.score_raw,
    case when v_result_type = 'not_reported' then null else p_score end,
    'closed',
    uid,
    m_old.status
  );
...
begin
  ...
  if v_status <> 'cancelled' then
    if p_result_type in ('not_reported', 'double_penalty', 'pending_score', 'retired_draw') then
      null;
  ...
  set
    ...
    winner_id = case
      ...
      when p_result_type in ('not_reported', 'double_penalty', 'pending_score', 'retired_draw') then null
      else p_winner_id
    end,
