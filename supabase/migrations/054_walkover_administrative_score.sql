-- W.O. now counts games as 6-3, 6-3 for the declared winner.

create or replace function public.walkover_score_for_winner(
  p_winner_id uuid,
  p_player_a_id uuid,
  p_player_b_id uuid
)
...
as $$
  select case
    when p_winner_id = p_player_a_id then
      '[{"a":6,"b":3},{"a":6,"b":3}]'::jsonb
    when p_winner_id = p_player_b_id then
      '[{"a":3,"b":6},{"a":3,"b":6}]'::jsonb
    else
      null::jsonb
  end;
$$;
...
begin
  ...
  if v_result_type = 'normal' and v_game_type = 'sudden_death' then
    if v_score is null or jsonb_typeof(v_score) <> 'array' or jsonb_array_length(v_score) <> 3 then
...
begin
  ...
  if p_result_type = 'wo' then
    v_score := public.walkover_score_for_winner(p_winner_id, m.player_a_id, m.player_b_id);
  end if;

  if p_result_type = 'normal'
     and p_game_type = 'sudden_death'
     and v_score is not null
     and jsonb_typeof(v_score) = 'array'
     and jsonb_array_length(v_score) = 3
  ...
  v_score_changed :=
    v_score is distinct from m.score_raw
