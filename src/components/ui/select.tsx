...
/** Texto legible para el trigger cuando `value` es un id y los hijos son JSX (Base UI usa `label` para eso). */
function extractPlainLabel(children: React.ReactNode): string | undefined {
  ...
  if (Array.isArray(children)) {
    ...
    for (const child of children) {
      const fragment = extractPlainLabel(child)
  ...
  if (React.isValidElement(children)) {
    if (children.type === React.Fragment) {
      ...
      return extractPlainLabel(nested)
    }
    const { children: nested } = children.props as { children?: React.ReactNode }
    return extractPlainLabel(nested)
  }
...
function SelectItem({
  ...
  const labelForValue = label ?? extractPlainLabel(children) ?? ""
  return (
